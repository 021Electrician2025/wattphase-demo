...
import { generateJobTicketId, anpostLocation } from "@/lib/mock-data";
...
export default function AnPostConfirmationPage() {
  ...
  const [jobTicket] = useState<JobTicket>({
    ...
    assignedElectrician: state.selectedElectrician || {
      ...
      isAvailable: true,
      location: { lat: 51.6227, lng: -8.8911 },
    },
    createdAt: new Date(),
    estimatedCompletion: new Date(Date.now() + 2 * 60 * 60 * 1000), // 2 hours from now
    description:
      "Critical power loss detected on Phase 2 affecting mail sorting operations at AnPost Clonakilty facility.",
    location: {
      lat: anpostLocation.lat,
      lng: anpostLocation.lng,
      address: anpostLocation.address,
    },
  });
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-green-800">
                  Electrician Successfully Dispatched to AnPost Clonakilty!
                </h2>
                <p className="text-sm sm:text-base text-green-700">
                  Your emergency request has been processed and an electrician
                  is on the way to restore power operations.
                </p>
              </div>
            </div>
          </CardContent>
            ...
            {currentStatus === "completed" && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-3 sm:p-4">
                ...
                <p className="text-xs sm:text-sm text-green-700">
                  The electrical issue has been resolved at AnPost Clonakilty. Power has been
                  restored to all phases and mail sorting operations can resume. A detailed report will be sent to your
